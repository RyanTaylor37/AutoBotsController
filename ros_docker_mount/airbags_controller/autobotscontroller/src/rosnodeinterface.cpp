...
// C/C++ source code generated on : Thu Nov 30 11:27:43 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"autobotscontroller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(autobotscontroller_M, (NULL));
        autobotscontroller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(autobotscontroller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(autobotscontroller_M)
                == (NULL)) || rtmGetStopRequested(autobotscontroller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(autobotscontroller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
